...
export class Order {
  ...
  private verifyMaxItemIsValid(createOrderCommand: CreateOrderCommand) {
    if (createOrderCommand.items.length > Order.MAX_ITEMS) {
      throw new BadRequestException(
        'Cannot create order with more than ' + Order.MAX_ITEMS + ' items',
      );
    }
  }
  ...
  addItem(productId: string, productName: string, price: number, quantity: number) {
    if (this.status !== OrderStatus.PENDING) {
      throw new Error('Commande déjà payée');
    }
    ...
    this.orderItems.push(
      new OrderItem({ productId, price, quantity }),
    );
  }

  public applyPromotion(promotionAmount: number): void {
    if (this.status !== 'PENDING') {
      throw new BadRequestException('Cannot apply a promotion to an order that is not pending.');
    }

    if (promotionAmount > this.price) {
      throw new BadRequestException('Promotion amount cannot exceed the total price.');
    }
